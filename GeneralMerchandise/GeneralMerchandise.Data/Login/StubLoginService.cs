    ...
    internal class StubLoginService : LoginService
    {
        ...
        public override ILoginResult Login(string accountIdentifier, string password)
        {
            ...
            if (accountIdentifier == "admin")
            {
                ...
                {
                    return new LoginResult(AccountModel.Existing(123234, "admin", sampleSalt, sampleHash, Common.Type.AccessType.Administrator, true));
                }
                else return new LoginResult("Wrong password for admin");
            }
            else return new LoginResult($"No account found with username {accountIdentifier}.");
        }
